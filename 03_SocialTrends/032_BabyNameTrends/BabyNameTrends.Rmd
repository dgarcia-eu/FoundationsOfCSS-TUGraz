...
The lines show the results of linear regression for boys and girls separately, we will learn more about linear regression soon in this course. We find the same result as the PNIS article, a positive correlation between the number of unique baby names in a year and the average US temperature, even though we measure it as anomaly rather than raw Fahrenheit like in the original paper. In particular, we get a correlation coefficient of ```r round(cor(sdf$Earth.s.surface[sdf$gender=="M"], sdf$nnames[sdf$gender=="M"]),3)``` for boys and of ```r round(cor(sdf$Earth.s.surface[sdf$gender=="F"], sdf$nnames[sdf$gender=="F"]),3)``` for girls. But do not be deceived, this does not mean that climate change is causing baby name diversity. Both quantities have an upwards trend and this correlation is a result of that. If you want to dig more on this topic, you can run yourself a [Granger test](https://en.wikipedia.org/wiki/Granger_causality) and you will see how we do not have evidence that rising temperatures cause larger numbers of names in any of the genders.

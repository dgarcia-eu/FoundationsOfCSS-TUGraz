...
In the analysis of the correlation between GDP and FOI of countries, you can run a permutation test. Before permutations, the scatter plot and correlation between GDP and FOI looks like this:
```{r echo=F}
plot(mdata$FOI, mdata$GDP, xlab="GDP", ylab="FOI")
```

And after one permutation, they look like this:

```{r echo=F}
shufdata <- mdata[sample(nrow(mdata)),]
plot(shufdata$FOI, mdata$GDP, xlab="GDP", ylab="Shuffled FOI")
```
...
After that, we have the values of the correlation coefficient in the permuted samples and the empirical value in our dataset. We can compare them by plotting a histogram of the permuted values and a line for the value in our dataset:
...
We can calculate the one-sided p-value from the permutation test on the previous example is as low as $10^{-4}$.
